execute if score 16_blue data matches 1 run return fail

scoreboard players set 16_blue data 1
tellraw @a[scores={region=17}] {"translate":"area.16.switch","color": "yellow"}

setblock 3109 134 889 minecraft:fire destroy
...
function dah.sch:set {time:20,run:'tellraw @a[scores={region=17}] {"translate":"area.17.save.blue"}'}
