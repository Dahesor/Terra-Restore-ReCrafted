...
execute store result score #temp2 calculator run attribute @s generic.attack_speed base get 100
...
execute unless data storage sys:mod result[{id:"generic.attack_speed"}] run return run function sys:att/apply/lore/dmg/only_default

data modify storage temp run set value {trans:"attribute.name.generic.attack_speed",value:0,modify:0,mc:0,vc:0}

execute store result score @s calculator run data get storage sys:mod result[{id:"generic.attack_speed"}].amount 10
