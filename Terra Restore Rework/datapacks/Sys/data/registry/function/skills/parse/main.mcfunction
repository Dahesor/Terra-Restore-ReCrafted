...
data modify storage registry:skill data[0].lore set value [{translate:"mp_cost_l",with:[{text:"",color:"aqua"}]},"    ",{translate:"cd_l",with:[{text:"",color:"light_purple"}]}]

#显示名称
data modify storage temp run set value {id:""}
data modify storage temp run.id set from storage registry:skill data[0].id
