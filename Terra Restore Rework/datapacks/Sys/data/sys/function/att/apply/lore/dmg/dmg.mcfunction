execute store result score #temp calculator run data get storage sys:data item.components."minecraft:attribute_modifiers"[{id:"minecraft:base_attack_damage"}].amount 10
execute store result score #temp2 calculator run attribute @s attack_damage base get 10
scoreboard players operation #temp calculator += #temp2 calculator

execute unless data storage sys:mod result[{id:"attack_damage",operation:"add_value"}] run return run function sys:att/apply/lore/dmg/only_default

data modify storage temp run set value {trans:"attribute.name.attack_damage",value:0,modify:0,mc:0,vc:0}

execute store result score @s calculator run data get storage sys:mod result[{id:"attack_damage",operation:"add_value"}].amount 10
