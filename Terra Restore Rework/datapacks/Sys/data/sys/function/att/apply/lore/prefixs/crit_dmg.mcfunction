data modify storage data sum_text set value {translate:"smk",with:[{translate:"stats.CRIT_DMG",color:"light_purple"},{text:"",extra:[{text:""},"%"]}],italic:false}

data modify storage data accepts set value {status_total:0}
data modify storage data accepts.integer set from storage data pr.stats_effects.crit_damage.add
