data modify storage data selected_type set value {id:"leggings"}
data modify storage data selected_type.name set value {translate:"armor.legs"}
# 强制规定武器类型为护腿

function sys:att/apply/types/general_armor

#植入属性与说明
data modify storage data modifiers[].id set value "registry:weapon_type/leggings"
data modify storage data additional_atts[].id set value "registry:prefix/leggings"
data modify storage data modifiers append from storage data additional_atts[]
data modify storage data modifiers[].slot set value "legs"
